...
export default class SplitcoinContainer extends Web3Component<any> {
	...
	render() {
		...
		return (
			...
			<div>
			{this.state.factoryAddr == '' &&
				<TextField floatingLabelText="FactoryAddress" onChange={this.onFactoryUpdate}/>
			}
			</div>
			<CurrencySelectorContainer label="Deposit Currency" onCoinChange={this.depositTypeChange}/>
			<div>
			{this.state.depositCurrency.symbol != Ether.symbol &&
				<TextField floatingLabelText="Return Address" floatingLabelFixed={true} type="text" value={this.state.returnAddr} onChange={this.onReturnAddrUpdate}/> }
			</div>
			<h3>To these Accounts</h3>
