...
export default class ReferralsContainer extends Web3Component<Props> {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      let viewAddr =
        this.props.match.params.address || (await this.getAccount());
  ...
  async getReferrals(account: string) {
    /*
     *let getReferral = (index: number) =>
     *  factory.methods.referrals(account, index).call();
  ...
  async getContracts(account: string) {
    let getContract = (index: number) =>
      factory.methods.contracts(account, index).call();
  ...
  async updatePendingBalance() {
    ...
    let referContract = new this.web3.eth.Contract(
      splitInterface.abi,
      referralContractAddr
    );
  ...
  async populateReferralView(address: string) {
    ...
    for (let referredUser of referrals) {
      ...
      for (let contract of userContracts) {
        let splitContract = new this.web3.eth.Contract(
          splitInterface.abi,
          contract
        );
        let contractIsClaimable = await splitContract.methods
          .isClaimable()
          .call();
        if (contractIsClaimable) {
          let split = {} as Split;
          let referContractPendingBalance = await splitContract.methods
            .getClaimableBalance()
            .call({
              from: referralContractAddr
            });
          let contractBalance = Number(
            this.web3.utils.fromWei(referContractPendingBalance, 'ether')
          );
  ...
  async onClaimAll() {
    ...
    for (let splitIndex = 0; splitIndex < splits.length; splitIndex++) {
      ...
      await this.claimFor(split.to, referContract)
  ...
  async getReferContractAddr(account: string) {
    let referralContractIndex =
      (await factory.methods.referralContracts(account).call()) - 1;
    if (referralContractIndex > -1) {
      let referralContract = await factory.methods
        .contracts(account, referralContractIndex)
        .call();
        ...
        <div className="content App-intro">
          <div>
            <Card>
              ...
              <CardHeader
                style={styles.header}
                title="Splitcoin - Referrals"
                subtitle="Pull your referral bonuses into your referral contract"
              ...
              <CardActions>
                <RaisedButton
                  disabled={this.state.referBalance <= 0}
                  onClick={this.onClaimAll}
                  label={`Sweep Referrals: ${this.state.referBalance} ETH`}
                  primary={true}
                />
                <RaisedButton
                  disabled={this.state.pendingBalance <= 0}
                  onClick={this.onClaim}
                  label={`Claim Balance: ${this.state.pendingBalance} ETH`}
                  primary={true}
                />
              </CardActions>
            </Card>
