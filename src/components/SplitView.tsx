...
export default class SplitView extends Component<Props> {
  ...
  render() {
    let removeCol = this.props.removeSplit ? (
      <FlatButton
        onClick={this.props.removeSplit(this.props.split)}
        label="Remove"
        secondary={true}
      />
    ) : null;
    let claimAction =
      ...
      this.props.split &&
      this.props.split.balance &&
      this.props.split.balance > 0 ? (
        <FlatButton
          onClick={this.props.onClaim(this.props.split)}
          label={`Send: ${this.props.split.balance} ETH`}
          secondary={true}
        />
      ) : null;
    return (
      <TableRow>
        ...
        <TableRowColumn>
          ...
          />
          <FlatButton onClick={this.contractClick(this.props.split.to)}>
            {this.props.split.to}
          </FlatButton>
        </TableRowColumn>
        <TableRowColumn>
